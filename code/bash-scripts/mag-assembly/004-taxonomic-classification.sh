...
export LC_ALL=C
ani_thresholds=("95" "99")
for ani_threshold in "${ani_thresholds[@]}"; do
    echo "Running GTDB-TK classify_wf on dereplicated genomes (${ani_threshold}% ANI threshold)"
    echo "$(date +"%F %H:%M:%S")"
    gtdbtk classify_wf \
        --genome_dir "${drep_output_dir}"/sa_"${ani_threshold}"perc/dereplicated_genomes/ \
        --out_dir "${gtdbtk_output_dir}"/sa_"${ani_threshold}"perc_GTDBtk \
        -x fa \
        --skip_ani_screen \
        --cpus "${nthreads_gtdbtk}" \
        --pplacer_cpus "${nthreads_pplacer}"
    gunzip "${gtdbtk_output_dir}"/sa_"${ani_threshold}"perc_GTDBtk/align/*.gz
    # Run classify separately because pplacer didn't recognize gz files

    echo "$(date +"%F %H:%M:%S")"
    echo "Running GTDB-TK classify on dereplicated genomes (${ani_threshold}% ANI threshold)"
    gtdbtk classify --genome_dir "${drep_output_dir}"/sa_"${ani_threshold}"perc/dereplicated_genomes/ \
        --out_dir "${gtdbtk_output_dir}"/sa_"${ani_threshold}"perc_GTDBtk \
        --align_dir "${gtdbtk_output_dir}"/sa_"${ani_threshold}"perc_GTDBtk \
        -x fa \
        --skip_ani_screen \
        --cpus "${nthreads_gtdbtk}" \
        --pplacer_cpus "${nthreads_pplacer}"
...
for ani_threshold in "${ani_thresholds[@]}"; do
    echo "Running CompareM on dereplicated genomes (${ani_threshold}% ANI threshold)"
    echo "$(date +"%F %H:%M:%S")"
    mkdir -p "${comparem_output_dir}"/aai_output_sa"${ani_threshold}"perc
    comparem  aai_wf \
        --cpus "${nthreads_comparem}" \
        "${drep_output_dir}"/sa_"${ani_threshold}"perc/dereplicated_genomes \
        -x fa \
        "${comparem_output_dir}"/aai_output_sa"${ani_threshold}"perc
