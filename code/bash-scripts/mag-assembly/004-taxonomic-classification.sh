...
comparem_output_dir=output/mag_assembly/comparem_output

cd ${project_home_dir}
mkdir -p output/mag_assembly/gtdbtk_output
mkdir -p output/mag_assembly/comparem_output
...
gtdbtk classify_wf \
    --genome_dir "${drep_output_dir}"/"${drep_date_time}"_sa_95perc/dereplicated_genomes/ \
    --out_dir "${gtdbtk_output_dir}"/"${gtdbtk_date_time}"_sa_95perc_GTDBtk \
    -x fa \
    --skip_ani_screen \
    --cpus "${nthreads_sort}" \
    --pplacer_cpus "${pplacer_threads}"
    # --scratch_dir "${gtdbtk_output_dir}"/"${gtdbtk_date_time}"_sa_95perc_GTDBtk \
...
gunzip "${gtdbtk_output_dir}"/"${gtdbtk_date_time}"_sa_95perc_GTDBtk/align/*.gz
gtdbtk classify --genome_dir "${drep_output_dir}"/"${drep_date_time}"_sa_95perc/dereplicated_genomes/ \
    --out_dir "${gtdbtk_output_dir}"/"${gtdbtk_date_time}"_sa_95perc_GTDBtk \
    --align_dir "${gtdbtk_output_dir}"/"${gtdbtk_date_time}"_sa_95perc_GTDBtk \
    -x fa \
    --skip_ani_screen \
    --cpus "${nthreads_sort}" \
    --pplacer_cpus "${pplacer_threads}"
...
gtdbtk classify_wf \
    --genome_dir "${drep_output_dir}"/"${drep_date_time}"_sa_99perc/dereplicated_genomes/ \
    --out_dir "${gtdbtk_output_dir}"/"${gtdbtk_date_time}"_sa_99perc_GTDBtk \
    -x fa \
    --skip_ani_screen \
    --cpus "${nthreads_sort}" \
    --pplacer_cpus "${pplacer_threads}"
    # --scratch_dir "${gtdbtk_output_dir}"/"${gtdbtk_date_time}"_sa_99perc_GTDBtk \
 # Run classify separately because pplacer didn't recognize gz files
intermediate_date_time=$(date +"%F %H:%M:%S")
echo "${intermediate_date_time}"
echo "Running GTDB-TK classify on dereplicated genomes (99% ANI threshold)"
gunzip "${gtdbtk_output_dir}"/"${gtdbtk_date_time}"_sa_99perc_GTDBtk/align/*.gz
gtdbtk classify --genome_dir "${drep_output_dir}"/"${drep_date_time}"_sa_99perc/dereplicated_genomes/ \
    --out_dir "${gtdbtk_output_dir}"/"${gtdbtk_date_time}"_sa_99perc_GTDBtk \
    --align_dir "${gtdbtk_output_dir}"/"${gtdbtk_date_time}"_sa_99perc_GTDBtk \
    -x fa \
    --skip_ani_screen \
    --cpus "${nthreads_sort}" \
    --pplacer_cpus "${pplacer_threads}"
...
mkdir -p "${comparem_output_dir}"/"${comparem_date_time}"_aai_output_sa95perc
comparem  aai_wf \
 --cpus "${nthreads}" \
