...
awk 'FNR!=1'  "${coverm_output_dir}"/*_sa_95perc_coverm_output_unmap_fix.tsv | \
  ...
  "${coverm_output_dir}"/all_samples_sa_95perc_coverm.tsv
# Merge all 99% ANI bins
awk 'FNR!=1'  "${coverm_output_dir}"/*_sa_99perc_coverm_output_unmap_fix.tsv | \
  csvtk -t add-header -n genome,relative_abundance,mean,trimmed_mean,covered_fraction,covered_bases,variance,length,read_count,reads_per_base,rpkm,tpm > \
  "${coverm_output_dir}"/all_samples_sa_99perc_coverm.tsv
# Merge all metabat bins
awk 'FNR!=1'  "${coverm_output_dir}"/*_all_bins_coverm_output_unmap_fix.tsv | \
  csvtk -t add-header -n genome,relative_abundance,mean,trimmed_mean,covered_fraction,covered_bases,variance,length,read_count,reads_per_base,rpkm,tpm > \
  "${coverm_output_dir}"/all_samples_bins_coverm.tsv
# Merge all megahit contigs
awk 'FNR!=1'  "${coverm_output_dir}"/*_megahit_coverm_output_fix.tsv | \
  ...
  "${coverm_output_dir}"/all_samples_megahit_coverm.tsv
...
do 
 ...
 SAMPLE=$(echo "${base_name}" | sed "s/\.megahit_asm//" )
 contigs_fasta_file="${FILE_DIR}"/"${SAMPLE}"_final.contigs.fa
...
for BIN_FASTA_FILE in "${drep_output_dir}"/sa_95perc/dereplicated_genomes/*_bin.*.fa
do
 base_name=$(basename "${BIN_FASTA_FILE}" )
 base_name=$(echo "${base_name}" | sed "s/\.fa//")
 ...
 seqkit fx2tab -l -g -n -i -H -B AT "${BIN_FASTA_FILE}" | \
    csvtk -t -C '&'  rename -f "#id",length -n contig_id,contig_length | \
     awk -v bin_id="${BIN_ID}" -v sample="${SAMPLE}" 'BEGIN {FS=OFS="\t"}
      ...
      FNR>1 {print sample,bin_id,$0}'  > \
      "${seqkit_all_tsv_dir}"/sa_95perc_"${SAMPLE}"_bin."${BIN_ID}"_seqkit.tsv;
done
echo "Done"

# Also for metabat2 bins
mkdir -p "${seqkit_output_dir}"/metabat2_bins_all_tsv
seqkit_all_tsv_dir="${seqkit_output_dir}"/metabat2_bins_all_tsv

for BIN_FASTA_FILE in "${metabat2_output_dir}"/*bins/*bin.*.fa
do 
 base_name=$(basename "${BIN_FASTA_FILE}" )
 base_name=$(echo "${base_name}" | sed "s/\.fa//")
 SAMPLE=$(echo "${base_name}" | sed "s/_bin\.[0-9]*//")
 ...
 seqkit fx2tab -l -g -n -i -H -B AT "${BIN_FASTA_FILE}" | \
    csvtk -t -C '&'  rename -f "#id",length -n contig_id,contig_length | \
     awk -v bin_id="${BIN_ID}" -v sample="${SAMPLE}" 'BEGIN {FS=OFS="\t"}
      FNR==1 {print "sample","bin_id",$0};
      FNR>1 {print sample,bin_id,$0}'  > \
      "${seqkit_all_tsv_dir}"/metabat2_all_"${SAMPLE}"_bin."${BIN_ID}"_seqkit.tsv;
done
echo "Done"
...
# for FILE_DIR in "${metabat2_output_dir}"/*bins/*bin.*.fa
# do 
#  SAMPLE=$(echo "${FILE_DIR}"  | sed "s/_bins//")
#  base_name=$(basename "$SAMPLE" )
#  mkdir -p "${seqkit_output_dir}"/"${base_name}"_seqkit_stats
#  seqkit_stats_out_dir="${seqkit_output_dir}"/"${base_name}"_seqkit_stats
#  echo "Calculating statistics on ${base_name}" 
#   for BIN_FASTA_FILE in "${metabat2_output_dir}"/"${base_name}"_bins/*.fa
#   do
#     BIN_ID=$(basename "${BIN_FASTA_FILE}" )
#     BIN_ID=$(echo "${BIN_ID}" | sed "s/\.fa//")
#     seqkit fx2tab -l -g -n -i -H -B AT "${BIN_FASTA_FILE}" | \
#      csvtk -t -C '&'  rename -f "#id",length -n contig_id,contig_length \
...
awk FNR!=1 "${seqkit_all_tsv_dir}"/sa_95perc_*_seqkit.tsv | \
 csvtk -t add-header  -n sample,bin_id,contig_id,contig_length,GC,AT > \
  "${seqkit_output_dir}"/sa_95perc_combined_stats.tsv

# Do the same for MetaBAT2 stats (all bins)
seqkit_all_tsv_dir="${seqkit_output_dir}"/metabat2_bins_all_tsv
awk FNR!=1 "${seqkit_all_tsv_dir}"/metabat2_all_*_seqkit.tsv | \
 csvtk -t add-header  -n sample,bin_id,contig_id,contig_length,GC,AT > \
  "${seqkit_output_dir}"/metabat2_combined_stats.tsv

# Do the same for MEGAHIT stats (be careful of the different columns: no bin_id)
seqkit_all_tsv_dir="${seqkit_output_dir}"/megahit_all_tsv
awk FNR!=1 "${seqkit_all_tsv_dir}"/megahit_*_seqkit.tsv | \
 csvtk -t add-header  -n sample,contig_id,contig_length,GC,AT > \
  "${seqkit_output_dir}"/megahit_combined_stats.tsv

# Sanity check
wc_count=0
for FILE in "${seqkit_output_dir}"/sa_95perc_all_tsv/sa_95perc_*_seqkit.tsv
do 
 count=$(tail -n +2 $FILE  |wc -l )
 wc_count=$((wc_count + count))
done
echo $wc_count
tail -n +2 "${seqkit_output_dir}"/sa_95perc_combined_stats.tsv |wc -l

wc_count=0
for FILE in "${seqkit_output_dir}"/metabat2_bins_all_tsv/metabat2_all_*_seqkit.tsv
do 
 count=$(tail -n +2 $FILE  |wc -l )
 wc_count=$((wc_count + count))
done
echo $wc_count
tail -n +2 "${seqkit_output_dir}"/metabat2_combined_stats.tsv |wc -l

wc_count=0
for FILE in "${seqkit_output_dir}"/megahit_all_tsv/megahit_*_seqkit.tsv
do 
 count=$(tail -n +2 $FILE  |wc -l )
 wc_count=$((wc_count + count))
done
echo $wc_count
tail -n +2 "${seqkit_output_dir}"/megahit_combined_stats.tsv |wc -l
...
if [[ ${#stats_files[@]} -eq 0 ]]; then
    ...
    awk 'BEGIN {FS=OFS="\t"} 
        ...
        "${seqkit_output_dir}"/"${base_name}"_combined_stats.tsv
fi
echo "Done";
done
...
awk 'BEGIN {FS=OFS="\t"} 
    ...
    "${seqkit_output_dir}"/all_samples_stats.tsv
...
echo $wc_count
wc "${seqkit_output_dir}"/all_samples_stats.tsv


wc_count=0
for FILE in "${seqkit_all_tsv_dir}"/sa_95perc_*_seqkit.tsv
do 
 count=$(tail -n +2 $FILE  |wc -l )
 wc_count=$((wc_count + count))
done
echo $wc_count
wc -l "${seqkit_output_dir}"/all_samples_stats.tsv
...
awk 'BEGIN {FS=OFS="\t"} 
    FNR==1 && NR==1 {printf  "%s\t%s\t%s", "Sample_Bin", "MeanGC", "MeanAT"; 
      ...
      END {
        ...
        "${seqkit_output_dir}"/average_gc_at_bins.tsv

# Filter to keep rows with low GC (below 30) (column 2)
awk -v threshold=30 'NR==1 || $2 < threshold'  "${seqkit_output_dir}"/average_gc_at_bins.tsv \
        > "${seqkit_output_dir}"/low_gc_bins.tsv
