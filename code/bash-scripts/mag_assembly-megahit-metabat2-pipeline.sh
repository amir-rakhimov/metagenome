...
mkdir -p output/mag_assembly/metabat2_output
...
for FILE_DIR in "${megahit_output_dir}"/"${date_time}"*megahit_asm
do 
...
prodigal \
    -i "${megahit_output_dir}"/"${megahit_date_time}"_2D14.megahit_asm/"${megahit_date_time}"_2D14_final.contigs.fa \
    -o "${prodigal_output_dir}"/"${date_time}"_2D14_prodigal/"${date_time}"_2D14_prodigal_coords.gbk \
    -a "${prodigal_output_dir}"/"${date_time}"_2D14_prodigal/"${date_time}"_2D14_prodigal_proteins.faa \
    -p meta 2>&1 |tee "${prodigal_output_dir}"/"${date_time}"_2D14_prodigal_report.txt;


# 5. Calculate contig coverage and extract unassembled reads
# 5.1 Use BBmap and samtools
conda deactivate
conda activate qc-tools

# 5.2 Align reads with bbwrap.sh:
for FILE_DIR in "${megahit_output_dir}"/"${date_time}"*megahit_asm
do 
...
# 5.3 Output per contig coverage to cov.txt with pileup.sh:
...
# 5.4 Extract unmapped reads (SE to unmapped.se.fq and PE to unmapped.pe.fq):
for FILE in "${megahit_aligned_reads_dir}"/"${date_time}"*_mapped_and_unmapped.sam.gz
do 
 SAMPLE=$(echo "${FILE}" | sed "s/_mapped_and_unmapped\.sam\.gz//" |sed "s/${date_time}_//")
 base_name=$(basename "$SAMPLE" )
...
## 5. Keep mapped reads (both mapped to contigs)
# 5.1 To get only the mapped reads use the parameter F, which works like -v of grep and skips the alignments for a specific flag.
for FILE in "${megahit_aligned_reads_dir}"/"${date_time}"*_mapped_and_unmapped.sam.gz
do 
  SAMPLE=$(echo "${FILE}" | sed "s/_mapped_and_unmapped\.sam\.gz//" |sed "s/${date_time}_//")
  base_name=$(basename "$SAMPLE" )
...
### 5.2 Sort bam file by coordinate (no options used)
for FILE in "${megahit_aligned_reads_dir}"/"${date_time}"*_bothReadsMapped.bam
do 
...
## 5.3 Filter unmapped reads (unmapped to contigs)
...
## 5.4 Split paired-end reads into separated fastq files .._R1 .._R2
### sort bam file by read name ( -n ) to have paired reads next to each other 
### ("${nthreads}" parallel threads, each using up to 5G memory)
for FILE in "${megahit_aligned_reads_dir}"/"${date_time}"*bothReadsUnmapped.bam 
do 
...
# 5.5 Compress bam and sam files to save space
for FILE in "${megahit_aligned_reads_dir}"/"${megahit_date_time}"*_mapped_and_unmapped.sam.gz
do 
 SAMPLE=$(echo "${FILE}" | sed "s/_mapped_and_unmapped\.sam\.gz//" |sed "s/${date_time}_//")
 base_name=$(basename "$SAMPLE" )
 echo "gzipping ${base_name}"
 gzip -9 --best "${megahit_aligned_reads_dir}"/"${megahit_date_time}"_"${base_name}"_bothReadsMapped.bam
...
do 
 ...
 gzip -9 --best "${megahit_aligned_reads_dir}"/"${megahit_date_time}"_"${base_name}"_bothReadsMapped_sorted.bam;
done
...
do 
 ...
 metabat2 -i "${megahit_output_dir}"/"${megahit_date_time}"_"${base_name}".megahit_asm/"${megahit_date_time}"_"${base_name}"_final.contigs.fa \
    ...
    -v \
    2>&1 |tee "${metabat2_output_dir}"/"${date_time}"_"${base_name}"_metabat2_report.txt;
done
...
do 
 ...
 "${metaquast_script_dir}"/metaquast.py \
    "${metabat2_output_dir}"/"${date_time}"_"${base_name}"/"${date_time}"_"${base_name}"_bin* \
...
do 
 ...
 busco -i "${metabat2_output_dir}"/"${date_time}"_"${base_name}" \
    -m genome \
...
do 
 ...
 checkm2 predict --threads "${nthreads}" \
   -x fa \
   --input "${metabat2_output_dir}"/"${date_time}"_"${base_name}" \
