library(tidyverse)
# Gene detection and annotation analysis ####
# Figures: ####
# 4. Number of unique genes by annotation in total (kofamscan, dbcan, all) (**My Figure 2**)
# 4.1 Number of unique genes by annotation in each sample (kofamscan, dbcan, all) (**My Figure 2**)
# 5. Most abundant genes in total with and without filtering by prevalence (**My Figure 3**)
# 6. Most abundant CAZyme subclasses (**My Figure 4**)
# 7. KEGG Pathways (**My Figure 5**)
...
dbcan.output.dir<-file.path("output/mag_assembly/dbcan_output", 
                            paste(dbcan.date_time,"nr_dbcan_proteins",sep = "_"))
...
kofamscan.fname<-file.path(kofamscan.output.dir,
                           paste(kofamscan.date_time,"nr_prot_kofam_scan_top_hits.txt",sep="_"))
...
# PROKKA mapping between samples, contigs, and genes ####
...
head(kofamscan.df)
# Not all mmseqs clusters are in kofamscan.df, but all kofamscan.df are in clusters (obviously)
